...
function empRenderBracketReal(){
  ...
  function renderMatch(match){
    ...
    function flagHtml(name){
      if(!hasTF||!hasFH||!name||name==='Por definir') return '';
      var tf = TF[name];
      if(!tf) return '';
    ...
    return '<div class="emp-match" style="background:'+bg+';border:'+border+';margin-bottom:6px">'
      ...
      +'</div>'
      +'<div style="font-size:9px;color:#A6ACC4;margin-top:3px">M'+match.m+(match.date?' &middot; '+match.date:'')+'</div>'
      +'</div>';
  }
...
if 'BRACKET_REAL' in html:
    ...
    if start > 0 and end > 0:
        ...
        print("WARNING: no se pudo reemplazar bloque existente")
else:
