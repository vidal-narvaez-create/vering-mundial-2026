"""
patch_bracket.py - Actualizado 08/07/2026
Suiza 0(4)-0(3) Colombia confirmado.
Cuartos: Francia vs Marruecos, España vs Bélgica, Noruega vs Inglaterra, Argentina vs Suiza
"""
import re, sys, os
...
const BRACKET_REAL = {
  ...
  r16: [
    ...
    {m:96, a:'Suiza',         ga:'0(4)',b:'Colombia',       gb:'0(3)',date:'07/07'},
  ],
  qf: [
    {m:97, a:'Francia',       ga:null, b:'Marruecos',      gb:null, date:'09/07'},
    {m:98, a:'España',        ga:null, b:'Bélgica',        gb:null, date:'10/07'},
    {m:99, a:'Noruega',       ga:null, b:'Inglaterra',     gb:null, date:'11/07'},
    {m:100,a:'Argentina',     ga:null, b:'Suiza',          gb:null, date:'12/07'},
  ],
  sf: [
...
function empRenderBracketReal(){
  ...
  if(!container) return;
  if(subtitle) subtitle.textContent = 'Resultados confirmados · Actualizado 08/07/2026';
  ...
  function renderMatch(match){
    ...
    var scoreA = score?'<span style="font-size:11px;font-weight:900;color:#E53935;flex-shrink:0;padding:0 4px">'+match.ga+'</span>':'';
    var scoreB = score?'<span style="font-size:11px;font-weight:900;color:#E53935;flex-shrink:0;padding:0 4px">'+match.gb+'</span>':'';
    return '<div class="emp-match" style="background:'+bg+';border:'+border+';margin-bottom:6px">'
      +'<div style="display:flex;align-items:center;justify-content:space-between;gap:4px;padding:2px 0">'
        +'<div style="display:flex;align-items:center;gap:4px;flex:1;overflow:hidden">'+flagHtml(nameA)
      ...
      +'</div>'
      +'<div style="display:flex;align-items:center;justify-content:space-between;gap:4px;padding:2px 0">'
        +'<div style="display:flex;align-items:center;gap:4px;flex:1;overflow:hidden">'+flagHtml(nameB)
        ...
        +'</div>'+scoreB
      +'</div>'
      +'<div class="emp-match-meta">'+(match.num?'M'+match.m:'M'+match.m)+(match.date?' · '+match.date:'')+'</div>'
      +'</div>';
  }
